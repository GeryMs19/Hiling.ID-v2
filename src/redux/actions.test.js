...
describe("Actions", () => {
  ...
  it("Creating action for IP retrieval request", () => {
    const expectedAction = { type: "FETCH_IP_REQUEST" };
    expect(fetchIPRequest()).toEqual(expectedAction);
  });

  it("Creating action for successful IP retrieval", () => {
  ...
  it("Creating action for failed IP retrieval", () => {
  ...
  it("Dispatching fetch IP action when IP retrieval is successful", async () => {
    const data = { ip: "127.0.0.1" };
    const mock = mockAxios
      .onGet("https://ifconfig.me/all.json")
      .reply(200, data);
  ...
  it("Dispatching fetch IP action when IP retrieval fails", async () => {
